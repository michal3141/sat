...
void inspect_current_formula(int i) {
    if (i < nclauses) {
        ...
        if ((tmp_seen == 1 && first < 0) || (tmp_seen == -1 && first > 0)) {
            conflict_count += std::pow(2, nclauses - i - 1);
        } else {
            if (first > 0)
                seen[first_abs] = 1;
            else
...
void evaluate_formula() {
    ...
    int sat_paths_count = std::pow(2, nclauses) - conflict_count;
...
// Generating all 2-SAT formulas with nvars and nclauses
...
// Generating all 2-SAT formulas with distinct literal clauses 
...
// Generating random 2-SAT formula with nvars and nclauses
void gen_random() {
